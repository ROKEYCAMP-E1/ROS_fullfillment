...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.captured_image_publisher = self.create_publisher(Image, '/captured_image', 10)
    ...
    def listener_callback(self, msg):
        ...
        cv2.imwrite("detective_img",cv_image)
        detected_message = self.bridge.cv2_to_imgmsg(cv_image, encoding="bgr8")
        self.detected_image_publisher.publish(detected_message)
        ...
        if len(results[0].boxes) > 0:
            ...
            for i, box in enumerate(boxes):
                ...
                if center_x - 60 <= box_center_x <= center_x + 40:
                    ...
                    self.captured_image_publisher.publish(captured_message)
                else:
                    feedback_msg = String()
